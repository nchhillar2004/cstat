...
CliAction parseCliArgs(int argc, char *argv[], Config *config) {
	...
	for (int i = 1; i < argc; i++) {
		...
		else if ((strcmp(arg, "-max") == 0 || strcmp(arg, "--max-file-size") == 0) &&
			 i + 1 < argc)
			config->max_file_size = _parse_arg_value(argv[++i], arg);
		else if ((strcmp(arg, "-min") == 0 || strcmp(arg, "--min-file-size") == 0) &&
			 i + 1 < argc)
		...
		else if (strcmp(arg, "-p") == 0 || strcmp(arg, "--precise") == 0)
			config->precise_numbers = true;
	...
	if (config->max_file_size > CAP_FILE_SIZE) {
		printf(COLOR_RED "error: " COLOR_RESET
				 "value cannot exceed the maximum cap of %dMB\n",
		       CAP_FILE_SIZE);
		exit(1);
	}
	...
	if (config->min_file_size > config->max_file_size) {
		printf(COLOR_RED "error: " COLOR_RESET
				 "min file size cannot exceed the max file size limit\n");
		exit(1);
	}
...
void cliPrintHelp() {
	...
	printf("    -p          --precise                      Show precise numbers in output\n");
