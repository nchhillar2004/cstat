...
static void runScan(Config *config) {
	...
	double dirWalkerEnd = getTime();

	printf("found %s directories and %s files at \"%s\".\n",
	       _num_format(stats.dir + stats.dirIgnored, config->precise_numbers),
	       _num_format(stats.files + stats.filesIgnored, config->precise_numbers),
	       config->path);
	...
	if (config->display_logs) {
		printf("Dir Walker took %.4lfs\n", (double)(dirWalkerEnd - startTime));
		printf("Dir Walker took %.4lfs\n", (double)(dirWalkerEnd - startTime));
	}
